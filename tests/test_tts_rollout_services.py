...
def groups():
    return [[Sample(index=index, group_index=index // 2, prompt="hello", label="hello", metadata={"language": "english"}) for index in range(start, start + 2)] for start in (0, 2)]
...
def test_zero_weight_components_need_no_services():
    config = RewardConfig((RewardComponentConfig("noop", 1), RewardComponentConfig("judge", 0), RewardComponentConfig("wer", 0)))
...
def test_composite_does_not_renormalize_or_hide_service_failures(monkeypatch):
    service = ServiceConfig("http://reward.invalid", model="judge", tokenizer_path="/tokenizer", runtime=RuntimeConfig(max_retries=0))
    config = RewardConfig((RewardComponentConfig("wer", 0.4), RewardComponentConfig("judge", 0.6)), {"wer": service, "judge": service})
