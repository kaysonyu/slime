...
def test_job_plan_runs_preflight_without_submitting_or_leaking_secrets(tmp_path):
    ...
    (tmp_path / "model/config.json").write_text(
        json.dumps({"sglang_omni_compat": {"local_layout": "gpt2_qkv_interleaved_v1"}})
    )
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "examples/tts_grpo/launch.py"), "plan"], env=env, text=True, capture_output=True
    )
