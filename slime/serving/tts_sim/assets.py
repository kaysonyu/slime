...
SEED_TTS_EVAL_ROOT = Path("/inspire/ssd/project/embodied-multimodality/public/kyu/src/emb/seed-tts-eval")
...
def verify_canonical_checkpoint(path: Path = CANONICAL_CHECKPOINT) -> str:
    ...
    if actual != CANONICAL_CHECKPOINT_SHA256:
        raise ValueError(
            f"checkpoint SHA256 mismatch for {path}: " f"expected {CANONICAL_CHECKPOINT_SHA256}, got {actual}"
        )
    return actual
