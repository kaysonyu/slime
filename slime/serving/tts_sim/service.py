...
class EmbeddingService:
    ...
    def __init__(
        ...
        self.reference_cache = reference_cache if reference_cache is not None else ReferenceEmbeddingCache()
    ...
    def _infer_loaded(self, loaded: Sequence[LoadedAudio]) -> list[_Embedded]:
        ...
        for bucket, indices in groups.items():
            for start in range(0, len(indices), bucket.max_batch):
                ...
                batch, lengths = self.bucket_policy.pad([loaded[index].waveform for index in chunk], bucket)
    ...
    def _prepare_path(self, path: str, cache_reference: bool) -> tuple[
        _Embedded | None,
        LoadedAudio | None,
        CacheKey | None,
    ...
    def _embed_paths(self, paths: Sequence[str], *, cache_references: bool) -> list[_Embedded]:
        if not 1 <= len(paths) <= self.max_items_per_request:
            raise ValueError(f"request item count must be between 1 and {self.max_items_per_request}")
        ...
        prepared = self._audio_executor.map(lambda path: self._prepare_path(path, cache_references), unique_paths)
            ...
            try:
                ...
                for index, cache_key, cache_future, result in zip(
                    ...
                    if cache_key is not None and cache_future is not None:
                        self.reference_cache.publish(cache_key, cache_future, self._cache_value(result))
