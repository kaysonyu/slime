...
class DynamicBatchingBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self._worker = threading.Thread(target=self._run, name="embedding-dynamic-batcher", daemon=True)
        self._worker.start()
    ...
    def _oldest_width(self) -> int | None:
        populated = ((queue[0].enqueued_at, width) for width, queue in self._queues.items() if queue)
    ...
    def embed(self, waveforms: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        if waveforms.shape[0] > self.max_batch:
            raise ValueError(f"submitted batch {waveforms.shape[0]} exceeds dynamic max {self.max_batch}")
