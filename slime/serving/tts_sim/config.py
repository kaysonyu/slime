...
class Settings:
    ...
    def from_env(cls) -> Settings:
        ...
        if raw_devices is None:
            ...
            devices = tuple(device.strip() for device in raw_devices.split(",") if device.strip())

        expected_cuda_devices = _optional_positive_int("WAVLM_SIM_EXPECTED_CUDA_DEVICES")
        if expected_cuda_devices is not None:
            visible_cuda_devices = torch.cuda.device_count()
            if visible_cuda_devices != expected_cuda_devices:
                raise ValueError(
                    f"expected {expected_cuda_devices} visible CUDA devices, " f"found {visible_cuda_devices}"
                )
            expected_devices = tuple(f"cuda:{index}" for index in range(expected_cuda_devices))
        ...
        if raw_roots is None:
            ...
            allowed_roots = tuple(Path(part).resolve() for part in raw_roots.split(os.pathsep) if part)
