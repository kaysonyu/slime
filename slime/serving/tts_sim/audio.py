    ...
    def resolve_path(self, value: str | os.PathLike[str]) -> Path:
        ...
        if self.allowed_roots and not any(
            ...
            raise PathAccessError(
                f"resolved audio path {resolved} is outside configured allowlist roots: {roots}"
            )
        if resolved.suffix.lower() not in SUPPORTED_SUFFIXES:
            raise AudioDecodingError(
                f"unsupported audio extension {resolved.suffix!r}; expected WAV or FLAC"
            )
        return resolved
        ...
        *,
        ...
        if waveform.numel() < self.min_samples:
            raise AudioTooShortError(
                f"audio has {waveform.numel()} effective samples; minimum is {self.min_samples}"
            )
