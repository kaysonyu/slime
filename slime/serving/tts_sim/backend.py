...
from .assets import CANONICAL_CHECKPOINT, CANONICAL_CHECKPOINT_SHA256, EMBEDDING_DIMENSION, verify_canonical_checkpoint
...
class PyTorchEmbeddingBackend:
    ...
    def __init__(
        ...
        if self.device.type == "cuda" and not torch.cuda.is_available():
            raise RuntimeError(f"configured SIM device {self.device} requires CUDA, but CUDA is unavailable")

        self.checkpoint_sha256 = (
            verify_canonical_checkpoint(self.checkpoint) if verify_hash else CANONICAL_CHECKPOINT_SHA256
        )
