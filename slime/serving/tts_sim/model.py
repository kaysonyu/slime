...
def _in_export_mode() -> bool:
    ...
    return torch.jit.is_tracing() or torch.jit.is_scripting() or (compiler is not None and compiler.is_compiling())
...
class MultiheadAttention(nn.Module):
    ...
    def _relative_attention_forward(
        ...
        if self.gru_rel_pos:
            ...
            query_layer = query_layer.view(bsz, tgt_len, self.num_heads, self.q_head_dim).permute(0, 2, 1, 3)
        ...
        merged_attn_mask = rel_attn_mask if merged_attn_mask is None else merged_attn_mask + rel_attn_mask
        ...
        if key_padding_mask is not None:
            ...
            merged_attn_mask = key_padding_mask if merged_attn_mask is None else merged_attn_mask + key_padding_mask
    ...
    def _prepare_inputs(self, x: Tensor, lengths: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if not _in_export_mode():
            ...
            if lengths.size(0) != x.size(0):
                raise ValueError(f"Expected lengths to have batch size {x.size(0)}, got {lengths.size(0)}")
