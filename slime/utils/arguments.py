...
def parse_args(add_custom_arguments=None):
    ...
    def add_tts_arguments(parser):
        ...
        group.add_argument(
            "--wer-language",
            default="en",
            help="Default language for rows without language; en/zh or a canonical language name",
        )
        group.add_argument(
            "--asr-protocol",
            choices=["openai_audio_transcriptions", "qwen3_asr_chat_path"],
...
def configure_tts_args(args):
    ...
    if (
        builtin_reward
        and not preloaded_rollout(args)
        and (args.objective == "grpo" or any(not dataset.reward_config for dataset in args.eval_datasets))
    ):
