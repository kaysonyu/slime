...
def audio_identity(path: Path) -> dict:
    ...
    stat = resolved.stat()
    return dict(path=str(resolved), size=stat.st_size, mtime_ns=stat.st_mtime_ns, frames=info.frames, sample_rate=info.samplerate, channels=info.channels)
...
def load_speech_manifest(path, *, language="en", reward_config=None, hf_checkpoint=None, metadata_overrides=None, teacher_domains=None):
    source = Path(path)
    raw = source.read_bytes()
    digest = hashlib.sha256(raw)
    digest.update(b"speech-manifest-v2")
    contract = dict(language=language, reward=reward_config.identity if reward_config else None, metadata_overrides=metadata_overrides or {}, teacher_domains=sorted(teacher_domains or ()))
    digest.update(json.dumps(contract, sort_keys=True).encode())
    if hf_checkpoint:
        root = Path(hf_checkpoint)
        names = {"config.json", "tokenizer_config.json", "tokenizer.json", "special_tokens_map.json", "preprocessor_config.json"}
    ...
    for number, line in enumerate(raw.decode("utf-8").splitlines(), 1):
        ...
        if isinstance(text, tuple):
            if any(item.local_instruction is not None for item in text):
                raise ValueError("Local generation does not support per-script local_instruction; provide global instructions")
