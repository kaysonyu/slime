...
class RolloutDataSource:
    ...
    def load(self, rollout_id=None):
        ...
        if "state_version" not in state_dict:
            ...
            logger.warning("Restoring a legacy sampler: only JSONL/seed identity is available; WER now uses the migrated definition")
        elif state_dict["state_version"] != 2 or state_dict.get("n_samples_per_prompt") != self.args.n_samples_per_prompt:
        ...
        if any(type(value) is not int or value < 0 for value in (self.sample_offset, self.epoch_id, self.sample_group_index, self.sample_index)):
