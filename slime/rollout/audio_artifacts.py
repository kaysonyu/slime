...
def persist_audio(audio, *, root, namespace, rollout_id, sample_id, attempt=0):
    ...
    return str(destination), dict(audio_sha256=hashlib.sha256(data).hexdigest(), sample_rate=rate, audio_channels=channels, audio_frames=frames, artifact_namespace=namespace)
