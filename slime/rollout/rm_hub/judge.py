...
DEFAULT_JUDGE_TOKENIZER_PATH: Final = (
...
def parse_sample_rubrics(sample: Sample) -> tuple[RubricItem, ...]:
    ...
    for index, raw_item in enumerate(raw_rubrics):
        ...
        if len(normalized_dimension) > MAX_RUBRIC_DIMENSION_CHARS:
            raise ValueError(
                f"Judge reward requires {item_name}.dimension to contain at most {MAX_RUBRIC_DIMENSION_CHARS} Unicode characters."
            )
        ...
        if len(normalized_statement) > MAX_RUBRIC_STATEMENT_CHARS:
            raise ValueError(
                f"Judge reward requires {item_name}.statement to contain at most {MAX_RUBRIC_STATEMENT_CHARS} Unicode characters."
            )
