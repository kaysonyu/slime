...
from .config import ComponentReward, get_reward_config
...
async def _score_sample(
    ...
    reference_result: (
        reference_similarity.ReferenceSimilarityResult
        | reference_similarity.GeneratedAudioFailure
        | sim.SimBatchFailure
        | None
    ),
    args: Namespace,
...
async def reward_batch(args: Namespace, samples: list[Sample], **_kwargs) -> list[float]:
    ...
    requires_http_runtime = (
        config.weight("wer") > 0
        or config.weight("judge") > 0
        or reference_similarity.requires_timbre_service(prepared_references)
    )
    if requires_http_runtime:
        ...
        if reference_active:
            reference_results: list[
                ...
                | sim.SimBatchFailure
                | None
            ] = list(await reference_similarity.score_prepared_batch(prepared_references, runtime))
        else:
            reference_results = [None] * len(samples)
        results = await asyncio.gather(
            *(
                _score_sample(sample, runtime, reference_result, args)
                for sample, reference_result in zip(samples, reference_results, strict=True)
            ),
        ...
        results = await asyncio.gather(
            *(
                _score_sample(sample, None, reference_result, args)
                for sample, reference_result in zip(samples, reference_results, strict=True)
            ),
            return_exceptions=True,
        )
    ...
    for sample, result in zip(samples, results, strict=True):
        ...
        value = sum(
            component.weight * by_name[component.name].reward
            for component in config.components
            if component.weight > 0
        )
        ...
        sample.metadata["reward_components"] = {
            name: {"reward": component.reward, "raw": component.raw_value} for name, component in by_name.items()
        }
        rewards.append(float(value))
    if failure_count:
        rendered_categories = ", ".join(
            f"{category}={count}" for category, count in sorted(failure_categories.items())
        )
        logger.warning(
