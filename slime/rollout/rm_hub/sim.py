...
async def _score_chunk(
    runtime: RewardHttpRuntime, items: Sequence[_SimilarityItem]
) -> list[tuple[int, ComponentReward]]:
...
async def score_batch(
    samples: Sequence[Sample], runtime: RewardHttpRuntime
) -> list[ComponentReward | SimBatchFailure]:
    ...
    chunks = [
        items[start : start + MAX_SIM_ITEMS_PER_REQUEST]
        for items in grouped_items.values()
        for start in range(0, len(items), MAX_SIM_ITEMS_PER_REQUEST)
    ]
