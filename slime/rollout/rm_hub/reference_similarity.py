...
def prepare_batch(samples: Sequence[Sample]) -> tuple[PreparedReferenceSample | GeneratedAudioFailure | ReferenceSimilarityResult, ...]:
    ...
    for sample in samples:
        ...
        required_uses = tuple(use for use in REFERENCE_AUDIO_USE_ORDER if any(use in reference.uses for reference in references))
        ...
        if generated_audio is None and sample.trajectory is not None and sample.trajectory.num_frames == 0:
            prepared.append(ReferenceSimilarityResult(ComponentReward("reference_similarity", 0.0, 0.0), (), required_uses))
            continue
...
async def score_prepared_batch(
    ...
    timbre_positions = [index for index, item in enumerate(prepared) if isinstance(item, PreparedReferenceSample) and "timbre" in item.required_uses]
...
def apply_diagnostics(sample: Sample, result: ReferenceSimilarityResult) -> None:
    if not isinstance(sample.metadata, dict):
        sample.metadata = {}
    sample.metadata["reference_similarity_dimensions"] = {dimension.use: dimension.to_metadata() for dimension in result.dimensions}
